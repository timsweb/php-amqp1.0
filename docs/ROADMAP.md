...
## ~~Unified Consumer Message Interface~~ ✅ Implemented (2026-03-15)

Implemented as `InboundMessage` — a readonly class that composes `Message` and `DeliveryContext` into a single object, delegating all read accessors and settlement methods:

```php
$client->consume($queue)->handle(function (InboundMessage $msg) {
    echo $msg->body();
    $msg->accept();
})->run();
```